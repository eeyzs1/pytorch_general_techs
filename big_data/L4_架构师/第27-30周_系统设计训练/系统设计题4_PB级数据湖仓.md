...
> **本设计题涉及矛盾**：矛盾1（容量/分片）+ 矛盾8（Schema/湖仓）。

PB级数据湖仓的设计本质是在矛盾1（如何存储和分片 PB 级数据）和矛盾8（如何管理 Schema 和事务）之间找到工程最优解。Iceberg/Hudi 提供了"开放格式 + 元数据层"的湖仓方案，用 Parquet 列存（矛盾7的存储引擎优化）+ 事务日志（矛盾8的 Schema 管理）消解了数据湖和数据仓库的分裂。冷热分层（Hot/Warm/Cold）则是对矛盾1的存储成本优化——把不常访问的数据迁移到廉价存储。

> **框架映射**：详见 [大数据第一性原理](../../大数据第一性原理.md) 矛盾1和矛盾8。