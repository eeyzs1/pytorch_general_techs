...
> **阶段周期**：8周主线（第17-24周）+ 补充模块按需 | **总课时**：320h主线 + 140h补充 | **项目数**：3个核心项目 + 6个补充项目 | **目标角色**：大数据中级工程师
...
8. **掌握多源数据清洗与实体对齐**：能把聊天记录、表单、邮件等多渠道非结构化数据清洗为结构化数据，能实现跨渠道用户身份打通（ID Mapping）和枚举值标准化，理解企业主数据管理（MDM）体系

---

## 第一性原理锚点

> 本阶段深入工程实践的核心矛盾——节点故障如何容错（矛盾4）、CAP一致性如何取舍（矛盾5）、延迟与吞吐如何平衡（矛盾6）、Schema灵活性与查询性能如何折中（矛盾8）。详见 [大数据第一性原理](../大数据第一性原理.md)。

| 本阶段课时 | 对应矛盾 | 原理深潜 |
|-----------|---------|---------|
| 课时17 Kafka架构 | 矛盾1+4+5+6 分片/容错/一致性/吞吐 | 原理深潜2（一致性与容错） |
| 课时18 Kafka EOS与事务 | 矛盾4+5 容错/一致性 | 原理深潜2（一致性与容错） |
| 课时19 Flink核心概念 | 矛盾6 延迟/吞吐 | 原理深潜4（流处理时间语义） |
| 课时20 Flink Checkpoint | 矛盾4 容错 | 原理深潜2（一致性与容错） |
| 课时22 Iceberg与Delta Lake | 矛盾8 Schema/湖仓 | 原理深潜1（存储引擎） |
...
| 周次 | 模块 | 内容 | 课时 | 核心产出 |
|------|------|------|------|----------|
| 第17-18周 | Kafka深入 | 架构原理 + Exactly-Once + 事务 | 40h | Kafka压测报告、事务Producer代码 |
| 第19-20周 | Flink流处理 | 核心API + Checkpoint + SQL CDC | 40h | Flink作业代码、CDC管道搭建 |
| 第21-24周 | DDIA精读 + 项目实战 | 读书会 + 3个综合项目 | 240h | 实时交易监控大屏、Kafka调优报告、故障排查SOP |
| 补充模块 | 数据湖仓实战 | Iceberg/Delta/Hudi实战 + 迁移项目 | 45h | Iceberg湖仓项目、迁移报告 |
| 补充模块 | 数据治理实操 | 数据质量 + 元数据 + 血缘 + 治理项目 | 40h | 数据治理平台、质量Dashboard |
| 补充模块 | 多源数据清洗与结构化 | 采集+清洗+ID Mapping+MDM+流水线 | 55h | 清洗流水线、ID Mapping表、统一反馈表 |

> **周次说明**：L1 阶段为第9-16周，L2 阶段从第17周开始。历史目录名（如 `第15-16周_Kafka深入`）保留不改，以周次表为准。
...
| 项目10 | 多渠道用户反馈清洗入仓 | 10h | Python+Spark+Union-Find+rapidfuzz+Airflow+GE | ★★★★★ |
...
- [模块说明](./补充模块_多源数据清洗与结构化/README.md)
- [课时24：多源异构数据采集与接入](./补充模块_多源数据清洗与结构化/课时24_多源异构数据采集与接入.md)
- [课时25：非结构化到结构化转换](./补充模块_多源数据清洗与结构化/课时25_非结构化到结构化转换.md)
- [课时26：数据清洗方法论与实战](./补充模块_多源数据清洗与结构化/课时26_数据清洗方法论与实战.md)
- [课时27：实体对齐与值标准化](./补充模块_多源数据清洗与结构化/课时27_实体对齐与值标准化.md)
- [课时28：文本实体消歧与主数据管理](./补充模块_多源数据清洗与结构化/课时28_文本实体消歧与主数据管理.md)
- [课时29：清洗流水线编排与质量校验](./补充模块_多源数据清洗与结构化/课时29_清洗流水线编排与质量校验.md)
- [项目10：多渠道用户反馈清洗入仓](./补充模块_多源数据清洗与结构化/项目10_多渠道用户反馈清洗入仓.md)
...
├── 补充模块_多源数据清洗与结构化/
│   ├── README.md                                ← 模块总览与学习路径
│   ├── 课时24_多源异构数据采集与接入.md          ← Sqoop/DataX/Flink CDC/邮件采集
│   ├── 课时25_非结构化到结构化转换.md            ← 聊天记录/邮件/JSON字段化
│   ├── 课时26_数据清洗方法论与实战.md            ← 六类清洗问题+Pipeline
│   ├── 课时27_实体对齐与值标准化.md              ← ID Mapping+枚举标准化（核心）
│   ├── 课时28_文本实体消歧与主数据管理.md        ← SimHash/MinHash+MDM体系
│   ├── 课时29_清洗流水线编排与质量校验.md        ← Airflow DAG+GE+dbt test
│   └── 项目10_多渠道用户反馈清洗入仓.md          ← 三源清洗端到端实战
├── 补充模块_跨行业实战/
