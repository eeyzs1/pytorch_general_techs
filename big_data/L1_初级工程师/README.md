...
> 本阶段直面大数据的核心矛盾——数据超出单机边界后如何存储（矛盾1）、如何计算（矛盾2）、Shuffle代价如何控制（矛盾3）、存储引擎如何选择（矛盾7）。详见 [大数据第一性原理](../大数据第一性原理.md)。

| 本阶段课时 | 对应矛盾 | 原理深潜 |
|-----------|---------|---------|
| 课时11 HDFS原理 | 矛盾1 容量/分片 | 原理深潜2（一致性与容错） |
| 课时12 MapReduce | 矛盾2 算力/并行 | 原理深潜3（分布式计算代价） |
| 课时13 Hive数据仓库 | 矛盾1+7+8 分片/存储引擎/Schema | 原理深潜1（存储引擎） |
| 课时14 Spark RDD | 矛盾2+3+4 算力/Shuffle/容错 | 原理深潜3（分布式计算代价） |
| 课时15 Spark SQL | 矛盾3 网络/Shuffle | 原理深潜3（分布式计算代价） |
| 课时16 Spark性能调优 | 矛盾3 网络/Shuffle | 原理深潜3（分布式计算代价） |
| 课时16.5 现代OLAP引擎 | 矛盾7 读写/存储引擎 | 原理深潜1（存储引擎） |
...
| 周次 | 模块 | 课时 | 学时 | 核心内容 |
|------|------|------|------|----------|
| 第9-10周 | Hadoop生态基础 | 课时11-13 | 48h | HDFS原理、MapReduce编程、Hive数据仓库 |
| 第11-12周 | Spark核心 | 课时14-16.5 | 48h | RDD核心原理、Spark SQL/DataFrame、性能调优、[课时16.5：现代OLAP引擎入门](./第9-10周_Spark核心/课时16.5_现代OLAP引擎入门.md) |
| 第13-16周 | 项目实战 | 项目4-6 | 96h | 离线数仓构建、用户画像系统、Airflow编排 |
| 第16周末 | 结业考核 | - | 8h | 笔试 + 机试 + 项目答辩 |

> **周次说明**：L0 阶段为第1-8周，L1 阶段从第9周开始。历史目录名（如 `第7-8周_Hadoop生态`）保留不改，以周次表为准。
