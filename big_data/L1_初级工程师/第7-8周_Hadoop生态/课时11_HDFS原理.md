...
HDFS 解的是**矛盾 1（单机容量有限→必须分片存储）**。它的每个设计决策都可以从这个矛盾推导出来：

- **为什么用 128MB 大块？** NameNode 的内存要存所有 Block 的元数据（每个约 150 字节）。块太小（如 4KB），1PB 数据需要 2.7 亿个 Block，NameNode 内存爆炸；块太大，并行度不足。128MB 是元数据压力与并行度的折中。
- **为什么 3 副本？** 2 副本不够安全（同时坏 2 块的概率不可忽略），4 副本浪费存储。3 副本在可靠性和成本间取得平衡。
- **为什么副本放置策略是机架感知（第1份本机架，第2-3份跨机架）？** 兼顾本地性（本机架副本供计算优先读取，解矛盾 2 的数据本地性）和容错性（跨机架防整个机架故障，解矛盾 4）。

**失败模式**：HDFS 不适合存小文件（元数据爆炸）和低延迟点查（大块设计为批量读取优化）。这正是矛盾 1 的权衡——HDFS 选了"大文件高吞吐"，放弃了"小文件低延迟"。

**延伸阅读**：[原理深潜2：一致性与容错](../../原理深潜/原理深潜2_一致性与容错.md)（副本与容错原理）