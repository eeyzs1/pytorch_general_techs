...
Hive 同时触及三个矛盾，是"多矛盾交叉解法"的典型：

- **矛盾 7（读写/存储引擎）——为什么数仓用 Parquet/ORC 列存？** 数仓的核心访问模式是 OLAP（大批量扫描聚合），列存只读需要的列（减 IO 90%+）、列内压缩率高（减存储）、向量化执行（利用 CPU 缓存/SIMD）。TextFile 只适合调试。详见原理深潜1。
- **矛盾 1（容量/分片）——为什么分区裁剪能减 IO？** Hive 分区把数据按维度（如日期）切成目录，查询时只扫目标分区。这是矛盾 1 分片的红利——分片不仅为了容量，还为了查询时只扫相关分片。
- **矛盾 8（Schema/湖仓）——为什么 Hive 是"严格 Schema"的代表？** Hive 表要预先定义 Schema，加字段/改类型痛苦。这正是矛盾 8 中"性能/治理 vs 灵活性"选择"性能/治理"的一端。Iceberg/Delta Lake 是为补这个短板而生（Schema 演进+ACID）。

**失败模式**：Hive 不适合低延迟交互查询（秒级以上），不适合频繁更新（列存重写代价大）。这正是 Presto/Trino 和 ClickHouse 补位的原因。

**延伸阅读**：[原理深潜1：存储引擎](../../原理深潜/原理深潜1_存储引擎.md)（列存与向量化原理）