...
> **本文验证的矛盾**：矛盾4（容错）+ 矛盾6（延迟/吞吐）。

Flink 证明了：流处理可以在"低延迟"和"Exactly-Once 容错"之间同时兼得。通过异步 Barrier 快照（Chandy-Lamport 算法的工程实现），Flink 在不阻塞数据流的前提下实现全局一致性快照，解决了矛盾4的容错问题，而 Barrier 随数据流流动的方式保证了毫秒级延迟（矛盾6）。这与 Spark Streaming 的"微批"方案形成对比——微批用批量换取吞吐但牺牲延迟，Flink 用真正的流处理在两者间找到了更优的平衡点。

> **框架映射**：详见 [大数据第一性原理](../../大数据第一性原理.md) 矛盾4和矛盾6。深度原理见 [原理深潜2：一致性与容错](../../原理深潜/原理深潜2_一致性与容错.md) 和 [原理深潜4：流处理时间语义](../../原理深潜/原理深潜4_流处理时间语义.md)。
