...
> **本文验证的矛盾**：矛盾2（算力/并行）+ 矛盾3（网络/Shuffle）。

MapReduce 证明了：分布式计算的最小抽象只需要两个原语——Map（并行化）和 Reduce（聚合）。Map 解决矛盾2的并行算力问题（把大任务拆成小任务并行执行），Reduce 解决聚合问题。但两者之间的桥梁是 Shuffle（矛盾3），这也是 MapReduce 最大的性能代价——中间结果落盘 + 网络传输。MapReduce 的设计权衡是"用 Shuffle 的代价换取编程模型的简洁性和容错性"，这一取舍启发了后续 Spark（内存中间结果减少 Shuffle 代价）和 Flink（流式 Shuffle 避免落盘）的演进。

> **框架映射**：详见 [大数据第一性原理](../../大数据第一性原理.md) 矛盾2和矛盾3。深度原理见 [原理深潜3：分布式计算代价](../../原理深潜/原理深潜3_分布式计算代价.md)。