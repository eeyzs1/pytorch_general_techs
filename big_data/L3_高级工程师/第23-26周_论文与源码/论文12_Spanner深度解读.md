...
Spanner 证明了：分布式系统的时间问题不是"无法解决"，而是"需要付出代价"——通过专用硬件（TrueTime API，GPS + 原子钟）把时钟误差降到毫秒级，再用 Commit Wait 等待误差窗口过去，就能实现外部一致性（线性化）。Spanner 在 CAP 中选择了 CP，但通过 TrueTime 让"强一致"的代价变得可接受，开创了全球强一致数据库的新范式。这与 Dynamo 的"最终一致"形成了矛盾5两端的经典对照：Dynamo 放弃一致性换取可用性，Spanner 用硬件投资换取强一致性——选择取决于业务需求。
