...
> **本文验证的矛盾**：矛盾2（算力/并行）+ 矛盾4（容错）。

RDD 证明了：分布式计算的容错不一定要靠"复制数据"，"记录如何重建"（Lineage）同样有效——前提是计算是确定性的、粗粒度的。这一洞察让中间结果可以留在内存（矛盾2的并行算力不再被 I/O 瓶颈拖累），同时通过 Lineage 重算解决矛盾4的容错问题，而无需数据复制的存储开销。窄依赖/宽依赖的区分则精确刻画了何时需要 Shuffle（矛盾3），使 DAG 调度器能自动识别 Stage 边界并 Pipeline 优化。

> **框架映射**：详见 [大数据第一性原理](../../大数据第一性原理.md) 矛盾2和矛盾4。深度原理见 [原理深潜3：分布式计算代价](../../原理深潜/原理深潜3_分布式计算代价.md)。
