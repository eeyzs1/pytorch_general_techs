...
> **本文验证的矛盾**：矛盾1（容量/分片）+ 矛盾7（读写/存储引擎，追加写优化）。

GFS 证明了：当数据超出单机边界，64MB 大块 + 3 副本是在"元数据压力"和"可靠性"之间的工程最优解。大块减少了 Master 的元数据内存占用（矛盾1的元数据膨胀问题），3 副本在可靠性和读吞吐间取得平衡。GFS 选择"追加写为主"（矛盾7的写优化选择），牺牲随机写性能换取极高的顺序写吞吐——这是日志结构存储的早期范本，影响了后续 Kafka、LSM-Tree 的设计。

> **框架映射**：详见 [大数据第一性原理](../../大数据第一性原理.md) 矛盾1和矛盾7。深度原理见 [原理深潜2：一致性与容错](../../原理深潜/原理深潜2_一致性与容错.md)。
