...
> **本文验证的矛盾**：矛盾5（CAP一致性）+ 矛盾7（读写/存储引擎，LSM-Tree）。

BigTable 证明了：LSM-Tree 是高写入吞吐场景下的最优存储引擎选择。通过 MemTable + SSTable + WAL 的组合，BigTable 将矛盾7的"写优化"推向极致——所有写都是顺序追加，牺牲读放大（需要合并多层 SSTable）换取极高写吞吐。在矛盾5方面，BigTable 选择单行强一致（同一行的读写都经过 Tablet Server），但放弃跨行事务，这是"在一致性和性能间做选择"的经典案例。LSM-Tree 的设计影响了 HBase、Cassandra、RocksDB 等几乎所有现代高性能存储引擎。

> **框架映射**：详见 [大数据第一性原理](../../大数据第一性原理.md) 矛盾5和矛盾7。深度原理见 [原理深潜1：存储引擎](../../原理深潜/原理深潜1_存储引擎.md) 和 [原理深潜2：一致性与容错](../../原理深潜/原理深潜2_一致性与容错.md)。