...
Raft 证明了：共识算法的核心挑战不是"数学正确性"（Paxos 早已解决），而是"可理解性"。Raft 通过问题分解（Leader选举 + 日志复制 + 安全性）和状态空间缩减，把复杂的一致性问题拆解为三个相对独立的子问题，使普通工程师也能正确实现和调试。在矛盾5的框架下，Raft 选择了 CP（强一致性 + 分区容错），通过 Leader-based 的设计简化了一致性推理。Raft 的成功说明：在分布式系统中，"简单到可理解"本身就是一种工程价值，因为能理解的系统才能被正确实现和运维。
